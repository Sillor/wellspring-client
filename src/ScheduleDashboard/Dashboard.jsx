...
function Dashboard(props) {
  ...
  const [date, setDate] = useState(
    todayDate.toLocaleDateString("en-US", dateFormating)
  );
      ...
      fetch("http://152.44.224.138:5174/appointments", {
        ...
        .then((appointments) => {
          ...
          appointments.forEach((patientAppointment) => {
            ...
            let dateFormat = patientAppointment.ScheduledDate.replace(
              "T00:00:00.000Z",
              ""
            );
            ...
            if (
              patientAppointment.Status === "open" &&
              localStorage.getItem("user") === patientAppointment.Username &&
      ...
      fetch("http://152.44.224.138:5174/labs", {
        ...
        .then((labs) => {
          ...
          labs.forEach((patientLab) => {
            ...
            if (
              patientLab.Status === "open" &&
              date === dayjs(dateFormat).format("dddd, MMMM D, YYYY")
            ) {
              fetch("http://152.44.224.138:5174/patient/", {
                method: "POST",
                headers: {
      ...
      fetch("http://152.44.224.138:5174/prescriptions", {
        ...
        .then((activePrescriptions) => {
          ...
          activePrescriptions.forEach((prescription) => {
            ...
            let dateFormat = prescription.OrderDate.replace(
              "T00:00:00.000Z",
              ""
            );
              ...
              fetch("http://152.44.224.138:5174/patient/", {
                ...
                .then((patient) => {
                  ...
                  const item = {
                    ...
                    title:
                      "Prescription Name: " + prescription.PrescriptionName,
  ...
  const resolve = (item) => {
    ...
    if (role === "Doctor") {
      result = confirm(
        "Resolving this will close out the current appointment. \n \n Are you sure you want to delete " +
          item.patientName +
          " 's appointment for " +
          item.time[0] +
          "?"
    ...
    // Dashboard update prescription
    else if (role === "Pharmacist") {
      result = confirm(
        "Resolving this will close out the current prescription, and add it to patient records. \n \n Are you sure you want to fulfill " +
          item.patientName +
          " 's prescription?"
      );
      console.log(item);
          ...
          <div key={event.id} className="border p-4 rounded-md mb-4">
            ...
            {selectedEvent && (
              <Popup trigger={ButtonPopup} setTrigger={setButtonPopup}>
                ...
                <button
                  className="flex mt-3 float-start  text-white border-slate-200 bg-red-500 hover:bg-red-300 h-10 w-36 justify-center text-center items-center rounded-lg"
                  onClick={() => resolve(selectedEvent)}
                >
                  Resolve
                </button>
                ...
                <button
                  onClick={() => {
                    navigate("/dashboard", {
                      state: {
                        selectedPatient: data.filter((patient) => {
                          return patient.id === selectedEvent.patientId;
                        }),
                      },
                    });
                  }}
                  className="flex float-end mt-3 border border-slate-200 bg-white hover:bg-slate-100 hover:text-slate-900 dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-800 dark:hover:text-slate-50 h-10 w-36 justify-center text-center items-center rounded-lg"
                >
                  Patient Chart
                </button>
              </Popup>
